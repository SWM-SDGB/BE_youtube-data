...
from src.youtube_live_video_list_crawling import get_video_urls_by_selenium
from src.youtube_livechat_crawling_nonBuffer import live_chat
from src.youtube_parsing_viewing_distribution import html_parsing

ray.init(num_cpus=10, dashboard_host='0.0.0.0',dashboard_port=8265, include_dashboard=True)
...
async def async_process(url,video_id,folder):
  await asyncio.gather(
      html_parsing(url, video_id, folder),
      live_chat(video_id, folder)
  )
